...
5. **CRS is large and not bundled.** The app downloads the structured reference string from `https://crs.aztec.network` into `<DocumentDir>/bb-crs/`. The circuit plan includes the outer (2^22) circuit, so `bn254_g1.dat` needs ~**256 MiB**. A fresh install must download it (the in-app downloader is single-shot, no resume), which can fail on flaky networks. See "Pre-seeding the CRS" below.
...
- **Server/prover Dockerfiles** (`Dockerfile`, `server-go/Dockerfile`) — easy to miss:
  ...
  - `ZKPASSPORT_PACKAGES_REF` = the `zkpassport-packages` commit whose `packages/zkpassport-utils/package.json` matches the SDK `utils` version. Find it via the commit history of that file (npm has no `gitHead`). The server's `aggregate-inputs.cjs`/`outer-inputs.cjs` load this build for outer input generation — a stale ref silently produces wrong outer inputs.
  - bb source build flags: bb 4.2 dropped `DISABLE_AZTEC_VM`; use `-DAVM=OFF` (option names live in `barretenberg/cpp/CMakeLists.txt` at the bb ref — verify they still exist after a bb major bump). `msgpack.cmake` is auto-fetched; the legacy `sed` patch is a no-op on 4.2.

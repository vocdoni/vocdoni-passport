...
5. **CRS is large and not bundled.** The app downloads the structured reference string from `https://crs.aztec.network` into `<DocumentDir>/bb-crs/`. Sizing is `(nextPow2(maxCircuitSize)+1)*64` per file over **all circuits in the request set**. Key gotcha: the request circuit set includes `outer_evm_count_N` (the **2^22** outer circuit), so even though the device does **not** prove the outer (the server does), `ensureCrsFilesForCircuits` still sizes the CRS to 2^22 → `bn254_g1.dat` ≈ **256 MiB** on the device. (The baseline-only startup set tops out at the DSC RSA‑4096 ≈ 2^19 ≈ 32 MiB; it's the per-request download that pulls the full 256 MiB.) A fresh install must download it (the in-app downloader is single-shot, no resume), which can fail on flaky networks. See "Pre-seeding the CRS" below. **Never read a CRS file whole into the JS heap** — the old `CRSManager` merged download parts via `RNFS.readFile(tmp,'base64')` + `Buffer.from`, loading the whole 256 MiB file into Hermes as a base64 string + Buffer (~300 MiB in one allocation) → `Failed to allocate a 313174712 byte allocation`, OOM past Hermes's 512 MiB JS-heap growth limit on a memory-constrained device. **This was the confirmed cause of the fresh-install proving OOM** (device-verified fixed 2026‑06). `CRSManager` now merges natively: `RNFS.moveFile` for a fresh download, chunked `RNFS.read`+`appendFile` (8 MiB) for a resume. It hid for a while because a **pre-seeded** CRS (debug `run-as` push) skips the download path entirely — it only fires on a clean install that actually downloads.
